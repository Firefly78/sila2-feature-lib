...
def format_docstring(description: str, indent: int = 4) -> str:
...
class SiLAXMLParser:
    ...
    def _parse_commands(self):
        ...
        for command in self.root.findall('sila:Command', self.ns):
            ...
            # Parse errors
            for error in command.findall('.//sila:Identifier', self.ns):
                if error.getparent().tag.endswith('DefinedExecutionErrors'):
                    cmd_info['errors'].append(error.text)
...
class FeatureGenerator:
    ...
    def generate_feature_ul(self) -> str:
        ...
        code_lines.append('    """')
        code_lines.extend(format_docstring(description, 4).split('\n'))
        code_lines.append('    """')
        code_lines.append("")
    ...
    def _generate_command(self, command: Dict) -> List[str]:
        ...
        errors_list = f"[{', '.join(command['errors'])}]" if command['errors'] else "[]"
    ...
    def _get_python_type(self, data_type_def: Dict) -> str:
        ...
        if data_type_def['type'] == 'basic':
            ...
            if 'min_inclusive' in constraints:
                return f"typing.Annotated[{base_type}, sila.constraints.MinimalInclusive({constraints['min_inclusive']})]"
            return base_type
...
class SiLATypesGenerator:
    ...
    def _get_python_type(self, data_type_def: Dict) -> str:
        ...
        if data_type_def['type'] == 'basic':
            ...
            constraints = data_type_def.get('constraints', {})
            if 'min_inclusive' in constraints:
                return f"typing.Annotated[{base_type}, sila.constraints.MinimalInclusive(value={constraints['min_inclusive']})]"
            return base_type
...
def main():
    ...
    if not args.feature_only:
        ...
        types_path = args.output_dir / "sila_types.py"
